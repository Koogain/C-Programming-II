/* 파일명: Assignmment_13.c
   ...
   내용: 정수의 개수 N을 입력받아 int가 N개 들어갈 수 있는 동적 메모리를 할당받는다. 이 배열에 임의의 정수를 N개 생성해서 채운 다음 텍스트 파일과 2진 파일로 각각
		 저장하는 프로그램을 작성하시오. 2진 파일로 저장할 때는 N을 먼저 저장하고, 배열의 내용을 저장하시오. 저장된 파일을 문서 편집기에서 열어서 내용을 확인해보고 파일의 크기도 비교해본다.
